import { useEffect, useState } from 'react';
import { Button, Card, Container } from 'react-bootstrap';
import { es } from 'date-fns/locale';
import { format, parseISO } from 'date-fns';
import jsPDF from 'jspdf';
import { UserOptions } from 'jspdf-autotable';
import 'jspdf-autotable';
import { WebpayResponseCertificado } from '../RespuestaWebPayCertificado';

declare module 'jspdf' {
    interface jsPDF {
        autoTable: (options: UserOptions) => void;
        lastAutoTable: {
            finalY: number;
        };
    }
}

interface Props {
    data: WebpayResponseCertificado;
}


interface ApoderadoData {
    primer_nombre: string;
    segundo_nombre: string;
    primer_apellido: string;
    segundo_apellido: string;
    rut: string;
    dv: string;
    telefono: string;
    correo_electronico: string;
}

function formatDate(isoString: any) {
    const date = parseISO(isoString);
    return format(date, "dd/MM/yyyy HH:mm:ss", { locale: es });
}

function formatNumber(number: any) {
    return number.toLocaleString('de-DE');
}

function getPaymentTypeDescription(paymentTypeCode: any) {
    switch (paymentTypeCode) {
        case 'VD':
            return 'Venta Débito';
        case 'VN':
            return 'Venta Normal';
        case 'VC':
            return 'Venta en Cuotas';
        case 'SI':
            return '3 Cuotas sin Interés';
        case 'S2':
            return '2 Cuotas sin Interés';
        case 'NC':
            return 'N Cuotas sin Interés';
        case 'VP':
            return 'Venta Prepago';
        default:
            return 'Tipo de Pago Desconocido';
    }
}

const fetchApoderadoData = async (rut: any): Promise<ApoderadoData> => {
    const response = await fetch(`https://colegio-backend.onrender.com/apoderado/rut/${rut}`);

    const data = await response.json();
    console.log(data);
    return {
        primer_nombre: data.primer_nombre_apoderado,
        segundo_nombre: data.segundo_nombre_apoderado,
        primer_apellido: data.primer_apellido_apoderado,
        segundo_apellido: data.segundo_apellido_apoderado,
        rut: data.rut,
        dv: data.dv,
        telefono: data.telefono_apoderado,
        correo_electronico: data.correo_apoderado
    };
};

function TransactionSuccessCertificado({ data }: Props) {
    const [apoderado, setApoderado] = useState<ApoderadoData | null>(null);

    console.log(data);

    useEffect(() => {
        const fetchBoletas = async () => {
            const ordenCompra = data.response.buy_order;
            const parts = ordenCompra!.split('-');
            console.log(parts);


            const apoderadoData = await fetchApoderadoData(data.response.session_id);
            console.log(apoderadoData);

            setApoderado(apoderadoData);



        };

        fetchBoletas();
    }, [data.response.buy_order]);

    const generatePdf = () => {
        const doc = new jsPDF({
            orientation: 'portrait',
            format: 'letter'
        });
    
        const logo = 'data:image/png;base64,';
    
        doc.addImage(logo, 'PNG', 15, 10, 20, 20);
    
        const apoderadoName = apoderado ? 
            `${apoderado.primer_nombre} ${apoderado.segundo_nombre} ${apoderado.primer_apellido} ${apoderado.segundo_apellido}`.trim() 
            : 'sin datos';
    
        doc.setFontSize(16);
        doc.setFont("helvetica", "bold");
        doc.text("Detalle de Transacción", 105, 20, { align: 'center' });
    
        doc.setFontSize(7);
        doc.text('Manuel Rodríguez #1064', 163, 15);
        doc.text('Los Andes, Valparaíso', 163, 18);
        doc.text('Teléfono: (342) 402858', 163, 22);
        doc.text('www.colegioandeschile.cl', 163, 26);
    
        // Agregar detalles de la factura
        doc.setFontSize(12);
        doc.text(`Comprobante de págo: 001-2024`, 15, 40);
        doc.text(`Fecha Documento: ${format(new Date(), 'dd/MM/yyyy')}`, 15, 45);
        doc.text(`Nombre Apoderado: ${apoderadoName}`, 15, 50);
    
        // Primera tabla: Información de la transacción
        const body = [
            ['Colegiatura', '$' + formatNumber(data.response.amount) || 'sin datos'],
            ['Orden de Compra', data.response.buy_order || 'sin datos'],
            ['Código de Autorización', data.response.authorization_code || 'sin datos'],
            ['Tipo de Pago Realizado', getPaymentTypeDescription(data.response.payment_type_code) || 'sin datos'],
            ['Fecha de Transacción', formatDate(data.response.transaction_date) || 'sin datos'],
            ['4 Últimos Dígitos de la Tarjeta', data.response.card_detail?.card_number || 'sin datos'],
        ];
    
        doc.autoTable({
            startY: 60,
            head: [['Descripción', 'Detalle']],
            body: body,
            theme: 'grid',
            pageBreak: 'auto'
        });
    
        let finalY = doc.lastAutoTable.finalY;
    
        // Segunda tabla: Lista de estudiantes certificados
        if (data.updatedRecords && data.updatedRecords.length > 0) {
            doc.setFontSize(12);
            doc.text(`Lista de Certificados`, 15, finalY + 10);
    
            const studentsBody = data.updatedRecords.map((record, index) => [
                index + 1,
                `${record.primerNombreAlumno} ${record.segundoNombreAlumno} ${record.primerApellidoAlumno} ${record.segundoApellidoAlumno}`,
                record.certificateType
            ]);
    
            doc.autoTable({
                startY: finalY + 15,
                head: [['#', 'Estudiante', 'Tipo de Certificado']],
                body: studentsBody,
                theme: 'grid',
                pageBreak: 'auto'
            });
    
            finalY = doc.lastAutoTable.finalY;
        }
    
        // Tercera tabla: Datos del apoderado
        if (apoderado) {
            doc.setFontSize(12);
            doc.text(`Detalle Apoderado`, 15, finalY + 10);
    
            const apoderadoBody = [
                ['Apoderado', apoderadoName],
                ['RUT Apoderado', `${apoderado.rut}-${apoderado.dv}`],
                ['Teléfono Apoderado', `+569 ${apoderado.telefono}`],
                ['Correo Apoderado', apoderado.correo_electronico]
            ];
    
            doc.autoTable({
                startY: finalY + 15,
                head: [['Campo', 'Detalle']],
                body: apoderadoBody,
                theme: 'grid',
                pageBreak: 'auto'
            });
    
            finalY = doc.lastAutoTable.finalY;
        }
    
        // Guardar el PDF
        doc.save('comprobante_pago.pdf');
    };
    

    return (
        <>
            <Card className='cardStyle shadow' style={{ backgroundColor: '#D4EDDA' }}>
                <Card.Header>
                    <Card.Title>Transacción Exitosa</Card.Title>
                </Card.Header>
                <Card.Body>
                    <Card.Subtitle><b>Monto:</b></Card.Subtitle>
                    <Card.Text><b>${formatNumber(data.response.amount)}</b></Card.Text>
                    <hr />

                    <Card.Subtitle><b>Descripción:</b></Card.Subtitle>
                    <Card.Text>
                        {data.updatedRecords.map((record, index) => (
                            <div key={index}>
                                <b>{record.certificateType}</b> - {record.primerNombreAlumno} {record.segundoNombreAlumno} {record.primerApellidoAlumno} {record.segundoApellidoAlumno}
                            </div>
                        ))}
                    </Card.Text>
                    <hr />

                    <Card.Subtitle><b>Orden de Compra:</b></Card.Subtitle>
                    <Card.Text><b>{data.response.buy_order}</b></Card.Text>
                    <hr />

                    <Card.Subtitle><b>Código de Autorización:</b></Card.Subtitle>
                    <Card.Text><b>{data.response.authorization_code}</b></Card.Text>
                    <hr />

                    <Card.Subtitle><b>Tipo de pago realizado:</b></Card.Subtitle>
                    <Card.Text><b>{getPaymentTypeDescription(data.response.payment_type_code)}</b></Card.Text>
                    <hr />

                    <Card.Subtitle><b>Fecha de Transacción:</b></Card.Subtitle>
                    <Card.Text><b>{formatDate(data.response.transaction_date)}</b></Card.Text>
                    <hr />

                    <Card.Subtitle><b>4 últimos dígitos de la tarjeta:</b></Card.Subtitle>
                    <Card.Text><b>{data.response.card_detail?.card_number}</b></Card.Text>
                    <hr />
                </Card.Body>
            </Card>
            <Container className='text-center mt-3'>
                <Button className='buttonAnimated' onClick={generatePdf}>Descargar PDF</Button>
            </Container>
        </>
    );
}

export default TransactionSuccessCertificado;
